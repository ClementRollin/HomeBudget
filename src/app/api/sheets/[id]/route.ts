...
export async function PUT(
  ...
  const updated = await sheetRepository.updateForFamily(session.familyId, id, parsed.data);
  if (!updated) {
    return NextResponse.json({ message: "Fiche introuvable" }, { status: 404 });
  }
...
export async function DELETE(
  ...
  const { id } = await context.params;
  const deleted = await sheetRepository.deleteForFamily(session.familyId, id);
  if (!deleted) {
    return NextResponse.json({ message: "Fiche introuvable" }, { status: 404 });
  }
